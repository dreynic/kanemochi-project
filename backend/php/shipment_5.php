...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($order_id) || empty($sent_date) || empty($sent_by)) {
        echo "<script>
    ...
    try {
        ...
        $query = "INSERT INTO shipment5 (order_id, sent_date, sent_by, submit) 
                  VALUES (?, ?, ?, ?)";
        $stmt = $pdo->prepare($query);

        // Execute the query with form data and timestamp
        if ($stmt->execute([$order_id, $sent_date, $sent_by, $submit])) {
            echo "<script>
